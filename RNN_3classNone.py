...
for build in range( n_folds ) :
    model = keras.Sequential(
        [
            layers.Bidirectional( tf.keras.layers.LSTM(units = units, return_sequences = True), input_shape = input_shape ),
            layers.Bidirectional( tf.keras.layers.LSTM(units = units) ),
