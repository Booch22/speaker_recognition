'''
!!! ไฟล ANN_3classNone.py ควรอยู่ที่เดียวกันกับโฟลเดอร์ DataSetLab เพื่อให้ง่ายต่อการเรียกใช้โฟลเดอร์ DataSetLab !!!
โมเดลนี้ใช้ชุดข้อมูลจากโฟลเดอร์ DataSetLab โดยจะใช้โฟลเดอร์ย่อยที่อยู่ในนั้น 2 ชุด คือ
1. TrainSet เป็นชุดข้อมูลสำหรับการเทรนด์โมเดล มี 45 ตัวอย่าง โดยแบ่งเป็น 3 โฟลเดอร์ย่อย คือ
    1.1 class_0 (เปิดประตู)
        มีจำนวน 20 ตัวอย่าง โดยจะแยกเป็นเสียงของผู้พูดคนที่ 1 จำนวน 10 ตัวอย่าง และผู้พูดคนที่ 2 จำนวน 10 ตัวอย่าง
    1.2 class_1 (ปิดประตู)
        มีจำนวน 20 ตัวอย่าง โดยจะแยกเป็นเสียงของผู้พูดคนที่ 1 จำนวน 10 ตัวอย่าง และผู้พูดคนที่ 2 จำนวน 10 ตัวอย่าง
    1.3 class_2 (เสียงพื้นหลัง)
        มีจำนวน 5 ตัวอย่าง
2. TestSet เป็นชุดข้อมูลสำหรับการทดสอบโมเดลหลังจากเทรนด์เสร็จ มี 25 ตัวอย่าง โดยแบ่งเป็น 3 โฟลเดอร์ย่อย คือ
    2.1 class_0 (เปิดประตู)
        มีจำนวน 10 ตัวอย่าง โดยจะแยกเป็นเสียงของผู้พูดคนที่ 1 จำนวน 5 ตัวอย่าง และผู้พูดคนที่ 2 จำนวน 5 ตัวอย่าง
    2.2 class_1 (ปิดประตู)
        มีจำนวน 10 ตัวอย่าง โดยจะแยกเป็นเสียงของผู้พูดคนที่ 1 จำนวน 5 ตัวอย่าง และผู้พูดคนที่ 2 จำนวน 5 ตัวอย่าง
    2.3 class_2 (เสียงพื้นหลัง)
        มีจำนวน 5 ตัวอย่าง
'''
...
def sort_data( feature, label ) :
    ...
    for i in range( n_folds ) : # 5 folds
        x_data[j1], x_data[j1 + 1] = feature[j2], feature[j2 + 1]        # Class 0 คนที่ 1
        x_data[j1 + 2], x_data[j1 + 3] = feature[j2 + 10], feature[j2 + 11]  # Class 0 คนที่ 2
        x_data[j1 + 4], x_data[j1 + 5] = feature[j2 + 20], feature[j2 + 21]  # Class 1 คนที่ 1
        x_data[j1 + 6], x_data[j1 + 7] = feature[j2 + 30], feature[j2 + 31]  # Class 1 คนที่ 2
...
def TurnPredicted( predicted ) :
    model_predicted = 0
    if np.max( predicted ) == predicted[0] :
        model_predicted = 0
    elif np.max( predicted ) == predicted[1] :
        model_predicted = 1
    elif np.max( predicted ) == predicted[2]:  
        model_predicted = 2
...
ConfusionMatrixDisplay.from_predictions( y_test, y_predicted, display_labels = [ "Open", "Close", "Background \nNoise" ]  )
plt.show()
